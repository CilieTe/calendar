...
cloud/
├── prisma/                  # 数据库模型
├── shared/                  # 📦 共享资源（assets, literaryData.ts, types.ts）
│   ├── assets/
│   │   ├── jieqi/          # 24节气图片
│   │   └── daily/          # 文学人物图片
...
**注意**: `shared/` 目录通过 `experimental.outputFileTracingIncludes` 配置确保被打包到 standalone 输出中。
...
将图片放入 `shared/assets/daily/`，命名为 `MM-DD.png` 或 `MM-DD.jpg`

### 节气图片

已内置 24 节气图片在 `shared/assets/jieqi/`
